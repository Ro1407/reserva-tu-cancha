...
import { updateCourt } from "@/lib/actions-CRUD";
...
interface UpdateCourtFormProps {
  actualCourt: CourtData;
  courtId: string;
}

export default function UpdateCourtForm({actualCourt, courtId}:UpdateCourtFormProps) {
  ...
  async function onSubmit(data: CourtData) {
    try {
      const result = await updateCourt(data, courtId);
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="text-center space-y-1">
            <CardTitle className="text-2xl font-bold">Editar Cancha</CardTitle>
            <CardTitle className="text-xl font-thin">
              Edita los campos para modificar la información de una cancha para un club deportivo.
            </CardTitle>
          </div>
        </CardHeader>
        <CardContent>
            ...
            {submitStatus != "idle" && (
              <div role="status" aria-live="polite" className="space-y-4">
                {submitStatus == "success" ? (
                  <FormMessage type={FormMessageType.success}>La cancha fue editada exitosamente</FormMessage>
                ) : (
                  <FormMessage type={FormMessageType.error}>
                    Ocurrió un error editando la cancha, por favor intente nuevamente
                  </FormMessage>
                )}
              </div>
            )}
